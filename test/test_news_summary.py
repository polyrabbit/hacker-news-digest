...
class NewsSummaryTestCase(TestCase):
    ...
    def test_small_text(self):
        ...
        summary, summarized_by = news.summarize()
    ...
    def test_fallback_when_new_fetch_failed(self):
        news = News(url='http://www.thehistoryblog.com/archives/67580')
        db.summary.add(news.url, 'wonderful summary', Model.OPENAI)
        news.content = 'The website is temporarily unable to service your request as it exceeded resource limit. Please try again later.'  # when site is overcrowded by HN users
        summary, summarized_by = news.summarize()
        self.assertEqual(summary, 'wonderful summary')
        self.assertEqual(summarized_by, Model.OPENAI)
